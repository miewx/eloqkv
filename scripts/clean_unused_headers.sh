...
if [ ! -f "$IWYU_BIN" ]; then
    echo "错误：未找到 include-what-you-use 诊断工具。该脚本必须在 Docker 容器内运行。"
    exit 1
fi

if [ ! -f "$FIX_INCLUDES_BIN" ]; then
    echo "错误：未找到 fix_include 头文件清理脚本。该脚本必须在 Docker 容器内运行。"
    exit 1
fi
...
if [ -n "$FILES" ]; then
    ...
    # --reorder: 对头文件进行重新排序 and 整理
    # --quoted_includes_first: 优先将双引号本地头文件排在前面
    python3 "$FIX_INCLUDES_BIN" --nosafe_headers --reorder --quoted_includes_first $FILES < "$IWYU_BUILD_DIR/iwyu.log"
else
