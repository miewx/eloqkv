...
NamespaceToken NamespaceStorage::GetToken(std::string_view ns)
{
    ...
    EloqKey db_key = EloqKey::Raw(std::string(kNamespacePrefix) + std::string(ns));

    GetCommand cmd;
    ObjectCommandTxRequest tx_req(server_->NamespaceTableName(), &db_key, &cmd, /*auto_commit=*/false, /*always_redirect=*/true, txm);
...
std::string NamespaceStorage::ByToken(const NamespaceToken &token, std::string &ns_id, uint64_t &epoch)
{
    ns_id = "";
    epoch = 1;
    ...
    EloqKey db_key = EloqKey::Raw(std::string(kTokenPrefix) + std::string(token.RawView()));
    ...
    if (success && cmd.result_.err_code_ == RD_OK)
    {
        ...
        i_key = std::make_unique<EloqKey>(EloqKey::Raw(std::string(kIdToNamePrefix) + encoded_id));

        cmd_i = std::make_unique<GetCommand>();
        tx_req_i = std::make_unique<ObjectCommandTxRequest>(server_->NamespaceTableName(), i_key.get(), cmd_i.get(), /*auto_commit=*/false, /*always_redirect=*/true, txm);
        ...
        e_key = std::make_unique<EloqKey>(EloqKey::Raw(std::string(kEpochPrefix) + encoded_id));
        cmd_e = std::make_unique<GetCommand>();
        tx_req_e = std::make_unique<ObjectCommandTxRequest>(server_->NamespaceTableName(), e_key.get(), cmd_e.get(), /*auto_commit=*/false, /*always_redirect=*/true, txm);
...
bool NamespaceStorage::Add(std::string_view ns, const NamespaceToken &token)
{
    ...
    EloqKey check_ns_key = EloqKey::Raw(std::string(kNamespacePrefix) + std::string(ns));
    EloqKey check_token_key = EloqKey::Raw(std::string(kTokenPrefix) + std::string(token.RawView()));

    GetCommand cmd_ns;
    ObjectCommandTxRequest tx_req_ns(server_->NamespaceTableName(), &check_ns_key, &cmd_ns, /*auto_commit=*/false, /*always_redirect=*/true, txm);
    ...
    ObjectCommandTxRequest tx_req_token(server_->NamespaceTableName(), &check_token_key, &cmd_token, /*auto_commit=*/false, /*always_redirect=*/true, txm);
    ...
    GetCommand cmd_id;
    ObjectCommandTxRequest tx_req_id(server_->NamespaceTableName(), &next_id_key, &cmd_id, /*auto_commit=*/false, /*always_redirect=*/true, txm);
    ...
    ObjectCommandTxRequest tx_req_set_ns(server_->NamespaceTableName(), &check_ns_key, &cmd_set_ns, /*auto_commit=*/false, /*always_redirect=*/true, txm);
    ...
    ObjectCommandTxRequest tx_req_set_token(server_->NamespaceTableName(), &check_token_key, &cmd_set_token, /*auto_commit=*/false, /*always_redirect=*/true, txm);
    ...
    ObjectCommandTxRequest tx_req_set_i(server_->NamespaceTableName(), &i_key, &cmd_set_i, /*auto_commit=*/false, /*always_redirect=*/true, txm);
    ...
    ObjectCommandTxRequest tx_req_set_e(server_->NamespaceTableName(), &e_key, &cmd_set_e, /*auto_commit=*/false, /*always_redirect=*/true, txm);
...
bool NamespaceStorage::Set(std::string_view ns, const NamespaceToken &token)
{
    ...
    if (cmd_token.result_.err_code_ == RD_OK)
    {
        ...
        owner_i_key = std::make_unique<EloqKey>(EloqKey::Raw(std::string(kIdToNamePrefix) + existing_encoded_id));

        cmd_owner = std::make_unique<GetCommand>();
        tx_req_owner = std::make_unique<ObjectCommandTxRequest>(server_->NamespaceTableName(), owner_i_key.get(), cmd_owner.get(), /*auto_commit=*/false, /*always_redirect=*/true, txm);
    ...
    if (cmd_ns.result_.err_code_ == RD_OK)
    {
        ...
        old_token_key = std::make_unique<EloqKey>(EloqKey::Raw(std::string(kTokenPrefix) + old_token));

        GetCommand cmd_old_t;
        ObjectCommandTxRequest tx_req_old_t(server_->NamespaceTableName(), old_token_key.get(), &cmd_old_t, /*auto_commit=*/false, /*always_redirect=*/true, txm);
        ...
        tx_req_del_old = std::make_unique<ObjectCommandTxRequest>(server_->NamespaceTableName(), old_token_key.get(), cmd_del_old.get(), /*auto_commit=*/false, /*always_redirect=*/true, txm);
...
bool NamespaceStorage::Del(std::string_view ns)
{
    ...
    std::unique_ptr<EloqKey> check_ns_key = std::make_unique<EloqKey>(EloqKey::Raw(std::string(kNamespacePrefix) + std::string(ns)));
    ...
    std::unique_ptr<ObjectCommandTxRequest> tx_req_ns = std::make_unique<ObjectCommandTxRequest>(
        server_->NamespaceTableName(), check_ns_key.get(), cmd_ns.get(), /*auto_commit=*/false, /*always_redirect=*/true, txm);
    ...
    check_token_key = std::make_unique<EloqKey>(EloqKey::Raw(std::string(kTokenPrefix) + token));
    ...
    std::unique_ptr<ObjectCommandTxRequest> tx_req_t = std::make_unique<ObjectCommandTxRequest>(
        server_->NamespaceTableName(), check_token_key.get(), cmd_t.get(), /*auto_commit=*/false, /*always_redirect=*/true, txm);
    ...
    if (!encoded_id.empty())
    {
        e_get_key = std::make_unique<EloqKey>(EloqKey::Raw(std::string(kEpochPrefix) + encoded_id));
        cmd_e = std::make_unique<GetCommand>();
        tx_req_e = std::make_unique<ObjectCommandTxRequest>(
            server_->NamespaceTableName(), e_get_key.get(), cmd_e.get(), /*auto_commit=*/false, /*always_redirect=*/true, txm);
        success = server_->ExecuteNamespaceTxRequest(txm, tx_req_e.get());
        if (success && cmd_e->result_.err_code_ == RD_OK)
        {
            try
        ...
        gc_key = std::make_unique<EloqKey>(EloqKey::Raw(std::string(kGcPrefix) + encoded_id + ":" + std::to_string(epoch)));
        set_gc_cmd = std::make_unique<SetCommand>("1");
        set_gc_req = std::make_unique<ObjectCommandTxRequest>(
            server_->NamespaceTableName(), gc_key.get(), set_gc_cmd.get(), /*auto_commit=*/false, /*always_redirect=*/true, txm);
    ...
    std::unique_ptr<ObjectCommandTxRequest> tx_req_del_ns = std::make_unique<ObjectCommandTxRequest>(
        server_->NamespaceTableName(), check_ns_key.get(), cmd_del_ns.get(), /*auto_commit=*/false, /*always_redirect=*/true, txm);
    ...
    std::unique_ptr<ObjectCommandTxRequest> tx_req_del_token = std::make_unique<ObjectCommandTxRequest>(
        server_->NamespaceTableName(), check_token_key.get(), cmd_del_token.get(), /*auto_commit=*/false, /*always_redirect=*/true, txm);
    ...
    if (!encoded_id.empty())
    {
        ...
        tx_req_del_i = std::make_unique<ObjectCommandTxRequest>(
            server_->NamespaceTableName(), i_key.get(), cmd_del_i.get(), /*auto_commit=*/false, /*always_redirect=*/true, txm);
        ...
        tx_req_del_e = std::make_unique<ObjectCommandTxRequest>(
            server_->NamespaceTableName(), e_key.get(), cmd_del_e.get(), /*auto_commit=*/false, /*always_redirect=*/true, txm);
...
std::map<NamespaceToken, std::string> NamespaceStorage::List()
{
    ...
    while (current_index < plan_size)
    {
        ...
        for (const auto &tuple : scan_batch)
        {
            ...
            if (full_key.size() > kNamespacePrefix.size() && full_key.substr(0, kNamespacePrefix.size()) == kNamespacePrefix)
            {
                ...
                auto get_req = std::make_unique<ObjectCommandTxRequest>(
                    server_->NamespaceTableName(), key_obj.get(), get_cmd.get(),
                    /*auto_commit=*/false, /*always_redirect=*/true, txm);

                if (server_->ExecuteNamespaceTxRequest(txm, get_req.get()) && get_cmd->result_.err_code_ == RD_OK)
                {
