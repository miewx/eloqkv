...
bool RedisServiceImpl::Init(brpc::Server &brpc_server)
{
    ...
    enable_namespace = config_reader.GetBoolean("local", "namespace", false);
...
std::string RedisServiceImpl::GetNamespaceTokenFromDB(std::string_view ns)
{
    ...
    current_namespace = "";
    EloqKey db_key("n:" + std::string(ns));
    current_namespace = std::move(old_ns);
...
std::string RedisServiceImpl::GetNamespaceFromTokenFromDB(std::string_view token, std::string &ns_id)
{
    ...
    std::string old_ns = std::move(current_namespace);
    current_namespace = "";
    EloqKey db_key("t:" + std::string(token));
    current_namespace = std::move(old_ns);
...
bool RedisServiceImpl::AddNamespaceToDB(std::string_view ns, std::string_view token)
{
    ...
    EloqKey check_token_key("t:" + std::string(token));
    current_namespace = std::move(old_ns);
...
bool RedisServiceImpl::SetNamespaceInDB(std::string_view ns, std::string_view token)
{
    ...
    if (cmd_token.result_.err_code_ == RD_OK)
    {
        ...
        old_ns = std::move(current_namespace);
        current_namespace = "";
        owner_i_key = std::make_unique<EloqKey>("i:" + existing_encoded_id);
        current_namespace = std::move(old_ns);
    ...
    if (cmd_ns.result_.err_code_ == RD_OK)
    {
        ...
        old_ns = std::move(current_namespace);
        current_namespace = "";
        EloqKey old_token_key("t:" + old_token);
        current_namespace = std::move(old_ns);
    ...
    if (encoded_id.empty())
    {
        old_ns = std::move(current_namespace);
        current_namespace = "";
        next_id_key = std::make_unique<EloqKey>("next_id");
        current_namespace = std::move(old_ns);
